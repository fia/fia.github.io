 ...
 * 时间格式化处理
 *
 * @param {string} fmt - "yyyy-MM-dd hh:mm:ss"需要的时间格式
 * @param {object} date - 时间对象
 * @returns {string} - 时间对象的对应格式
 */
...
class IdInput extends React.Component {
  ...
  exportData() {
    ...
    /**
     * 保存txt文件
     * 参考代码地址http://www.zhangxinxu.com/wordpress/2017/07/js-text-string-download-as-html-json-file/
     * 
    ...
    const now = new Date();
    funDownload(
      "条码扫码记录器数据备份_" + dateFtt("yyyy-MM-dd hh:mm:ss", now) + ".txt",
   ...
   */
  importData() {
    // 参考代码地址https://blog.csdn.net/zdavb/article/details/50266215
    let importData = "";
    const that = this;
      ...
      <div>
        ...
        <fieldset disabled="disabled">
          ...
          <ul>
            ...
            <li>- 不同电脑及不同浏览器可以通过原始备份导出数据，再导入备份数据完成，然后可以继续添加数据。</li>
            <li>- 支持导入多份数据，自动去除多份数据中重复部分。</li>
            <li>- 导入备份数据会合并当前电脑浏览器页面上的所有数据。</li>
            <li>- 推荐使用<a href="https://www.google.com/chrome/" target="_blank">Chrome浏览器</a>
            </li>
          </ul>
        </fieldset>
        <div>
        ...
        <QrIdTable qrIdList={this.state.idList} delButton={this.delQrId} />
      </div>
    );
  }
}
